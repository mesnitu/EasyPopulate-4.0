...
class ep4bookx extends base {
    ...
    const Project = 'ep4bookx';

    public $version = '0.9.9-rc2';
    public $tplForm;
    public $setFields = array();
    public $getFieldsNamesLenght = array();
    public $imagesToProcess = array();
    public $imagesPaths = array();
    public $authorImageFolder = '';
    public $bookx_product_type = null;
    public $ep4bookxManipulateImages = null;
    public $ep4bookx_enabled = null;
    public $bindStatus = array();
...
//                });
    ...
    function updateEP4Start(&$callingClass, $notifier, $paramsArray) {
        global $db, $curver, $ep4bookx_enabled, $ep4bookx_fields_conf, $bookx_product_type, $messageStack, $toogle_config, $toogle_text, $ep4bookx_msg;

        global $ep4bookx_default_cnf, $build_vars, $ep4bookx_configuration;

        $this->bookx_product_type = $bookx_product_type;
        ...
        $curver .= 'w/ ' . self::Project . ' ' . $this->version . ' ( <a href="' . zen_href_link(FILENAME_EASYPOPULATE_4, 'ep4bookx_action=ep4bookx_reloadConfig" class="ep4bookx-download-link" ', $request_type) . '" class="ep4bookx-download-link" >Reload Config</a> )';


        if ($ep4bookx_enabled == 1) {

            $this->ep4bookx_enabled = true;

            require(DIR_FS_ADMIN . DIR_WS_LANGUAGES . $_SESSION['language'] . '/easypopulate_4_bookx.php');
            //loop to get names lenght
            foreach ($ep4bookx_default_cnf->ep4bookx_default_fields as $key => $value) {

                $this->getFieldsNamesLenght[$key] = $value->length;
            }

            if (EP4BOOKX_MANIPULATE_IMAGES == true) {
                $this->ep4bookxManipulateImages = true;

                $this->imagesPaths = array(
                    "basePath" => DIR_FS_CATALOG . DIR_WS_IMAGES,
                    "srcFolder" => EP4BOOKX_IMAGES_SRC_TEMP_FOLDER,
                    "authorImagePath" => EP4BOOKX_AUTHORS_IMAGE_FOLDER_PATH,
                    "products_image_lenght" => zen_field_length(TABLE_PRODUCTS, 'products_image'),
                    "author_image_lenght" => zen_field_length(TABLE_PRODUCT_BOOKX_AUTHORS, 'author_image'),
                );

                //The Catalog Images folder
                $this->authorImageFolder = EP4BOOKX_AUTHORS_IMAGE_FOLDER;
            }
            // Some messages
            switch ($ep4bookx_msg) {
    ...
    function updateEP4CollationUTF8ZC13x(&$callingClass, $notifier, $paramsArray) {
        ...
        //pr($this->getFieldsNamesLenght, "this names lenght::: ");
        //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::
        // todo: for zc older versions
        //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    function updateEP4ZC155AfterHeader(&$callingClass, $notifier, $paramsArray) {
        global $ep4bookx_fields_conf, $ep4bookx_configuration;
        global $maintenance, $maintenance_state, $which_zc;

        $which_zc = PROJECT_VERSION_MINOR;
        if ($this->ep4bookx_enabled == 1) {
            // load header scripts			
            include_once EP4BOOKX_TPL_PATH . 'tpl_ep4bookx_header.php';
        }
    }
    ...
    function updateEP4DisplayStatus(&$callingClass, $notifier, $paramsArray) {
        ...
        echo EASYPOPULATE_4_DISPLAY_ENABLE_BOOKX . (($this->ep4bookx_enabled) ? '<font color="green">TRUE</font>' : "FALSE") . '<br />';
    }
    ...
    function updateEP4MaxLen(&$callingClass, $notifier, $paramsArray) {
        ...
        if ($this->ep4bookx_enabled == 1) {
            foreach ($this->getFieldsNamesLenght as $field_name => $lenght) {
                $l = strlen($field_name) - 17;
                echo substr($field_name, -(int) $l) . ': ' . $lenght . '<br/>';
            }
        }
    }
        ...
        if ((int) EASYPOPULATE_4_CONFIG_BOOKX_DATA == true) { // probably it could be removed
            ...
            if ($ep4bookx_flag_import == true) {
                ...
                if (isset($filelayout['v_bookx_publisher_name'])) {
                    ...
                    $this->bindStatus['bind_publisher'] = 1;
                ...
                if (isset($filelayout['v_bookx_binding_' . $epdlanguage_id])) {
                    ...
                    $this->bindStatus['bind_binding'] = 1;
                ...
                if (isset($filelayout['v_bookx_printing_' . $epdlanguage_id])) {
                    ...
                    $this->bindStatus['bind_printing'] = 1;
                ...
                if (isset($filelayout['v_bookx_condition_' . $epdlanguage_id])) {
                    ...
                    $this->bindStatus['bind_condition'] = 1;
                ...
                if (isset($filelayout['v_bookx_imprint_name'])) {
                    ...
                    $this->bindStatus['bind_imprint'] = 1;
                ...
                if (isset($filelayout['v_bookx_publishing_date'])) {
                    ...
                    $this->bindStatus['bind_publishing_date'] = 1;
...
//    }
    ...
    function updateEP4ExtraFunctionsSetFilelayoutCaseDefault(&$callingClass, $notifier, $paramsArray) {
        ...
        global $zco_notifier, $ep_dltype, $filelayout, $filelayout_sql, $langcode, $sql_filter, $ep4bookx_query, $ep4bookx_query_join, $build_vars;
        ...
        switch ($ep_dltype) {
			...
			FROM '
                ...
                $filelayout_sql .= ' WHERE p.products_type = ' . $this->bookx_product_type . ' AND p.products_id = ptoc.products_id 
    ...
    function updateEP4LinkSelectionEnd(&$callingClass, $notifier, $paramsArray) {
        global $request_type, $ep4bookx_customize_files, $ep4bookx_fields_conf, $toogle_config, $toogle_text;
        ...
        if ($this->ep4bookx_enabled == 1) {
            ...
            include_once EP4BOOKX_TPL_PATH . 'tpl_ep4bookx_fields.php';
        }
    }
...
//  $zco_notifier->notify('EP4_EXPORT_SPECIALS_AFTER');
    function updateEP4ExportSpecialsAfter(&$callingClass, $notifier, $paramsArray) {
        global $ep_dltype, $db, $filelayout_sql, $ep_uses_mysqli, $filelayout, $row, $langcode, $epdlanguage_id, $category_delimiter, $build_vars;

        if ($ep_dltype == 'bookx' && $this->ep4bookx_enabled == 1) {
...
//    }
    ...
    function updateEP4ImportAfterCategory(&$callingClass, $notifier, $paramsArray) {
        ...
        global $zco_notifier, $langcode, $epdlanguage_id, $edit_link, $ep4bookx_reports, $display_output, $ep_error_count, $ep_warning_count, $ep4bookx_extra_sqlwhere, $ep4bookx_extra_sqlcol, $ep4bookx_extra_sqlbind, $ep4bookx_flag_import, $build_vars, $categories_delimiter;
        ...
        global $v_products_image, $v_manufacturers_name, $v_products_name, $v_products_model;
...
// Master key exists in file, therefore can process information...
    ...
    function updateEP4ImportFileNewProductProductType(&$callingClass, $notifier, $paramsArray) {
        ...
        global $v_products_type, $v_artists_name;
        ...
        if ($this->ep4bookx_enabled == 1) {
            $v_products_type = $this->bookx_product_type;
        }
    }
...
//    }
    ...
    function updateEP4ImportFilePreDisplayOutput(&$callingClass, $notifier, $paramsArray) {
        ...
        if ($this->ep4bookxManipulateImages == true) {
            ...
            // The new width / 
            $nwidth = EP4BOOKX_BOOK_IMAGE_WIDTH;
            $nheight = EP4BOOKX_BOOK_IMAGE_HEIGHT;

            $processFromUrl = array();

            foreach ($this->imagesToProcess as $key => $process) {
               
                if ($process['destinationFolder'] == $this->authorImageFolder) {
                    $nwidth = EP4BOOKX_AUTHORS_IMAGE_WIDTH;
                    $nheight = EP4BOOKX_AUTHORS_IMAGE_HEIGHT;
                } else {
                    $nwidth = EP4BOOKX_BOOK_IMAGE_WIDTH;
                    $nheight = EP4BOOKX_BOOK_IMAGE_HEIGHT;
                }
            
                if (!file_exists($this->imagesPaths['basePath'] . $process['destinationFolder'])) {

                    mkdir($this->imagesPaths['basePath'] . $process['destinationFolder'], '0755', false);
                }
                ...
                if (!empty($process['url'])) {
                    ...
                    if (!is_file($this->imagesPaths['srcFolder'] . $process['tempName'] . $imgExt)) {

                        $ep4bookx_reports['Issues with image file'][] = '::FILE DOESN\'T EXISTS:: ' . $process['tempName'] . $imgExt . '<br>';
                    }

                    // Resize and move Local images to the destination Folder
                    resize_image('max', $this->imagesPaths['srcFolder'] . $process['tempName'] . $imgExt, $process['destinationPath'] . $imgExt, $nwidth, $nheight);
                }
            }
            clearstatcache();
          
            foreach ($processFromUrl as $processThis) {


                if ($processThis[2] == $this->authorImageFolder) {// if the author folder is present in the array
                    $nwidth = EP4BOOKX_AUTHORS_IMAGE_WIDTH;
                    $nheight = EP4BOOKX_AUTHORS_IMAGE_HEIGHT;
                }
                 ...
                 */
                download_file_from_url($processThis[0], $this->imagesPaths['srcFolder'] . $processThis[1]);
               
                resize_image('max', $this->imagesPaths['srcFolder'] . $processThis[1] . $imgExt, $processThis[3] . $imgExt, $nwidth, $nheight);
                //Check if the Images were downloaded
                if (!is_file($this->imagesPaths['srcFolder'] . $processThis[1] . $imgExt)) {

                    $ep4bookx_reports['Issues with image download'][] = '::ISSUES WITH IMAGE DOWNLOAD:: ' . $processThis[1] . $imgExt . '<br>';
                }
            }
        }
        ...
        if (!empty($ep4bookx_reports)) {
            $display_output .= '<table class="table table-hover""><caption>' . EASYPOPULATE_4_DISPLAY_BOOKX_REPORTS_BOOKX_HEADER . '</caption><tr class="bookx-reports-top"><th >Type</th><th>' . EASYPOPULATE_4_BOOKX_TABLE_BOOK . '</th></tr>';

            foreach ($ep4bookx_reports as $key => $value) {

                $display_output .= '<tr><th class="bookx-reports-th-left" rowspan ="' . (count($value) + 1) . '">' . $key . '</th>';
                $display_output .= '<th class="warning">' . EASYPOPULATE_4_BOOKX_TABLE_CAPTION . ' : ' . $key . '</th></tr>';
...
class ep4BookxVarsOverRide {
    ...
    public function ep4BookxRemove() {
    ...
    private function ep4BookxInstallConfig() {
        ...
        global $ep4bookx_install_configuration, $ep4bookx_install_default_fields_conf;

        $proj_prefix = ep4bookx::Project . '_';
        ...
        $ep4bookx_install_default_fields_conf = array(
            'ep4bookx_fields' => array(// All Fields 
            ...
            'ep4bookx_default_fields' => array(// The deafult fields to use
                ...
                'default_' . $proj_prefix . 'series_name' => array(
                    'name' => EP4BOOKX_FIELD_SERIES_NAME,
                    'value' => '',
                    'length' => zen_field_length(TABLE_PRODUCT_BOOKX_SERIES_DESCRIPTION, 'series_name')),
                'default_' . $proj_prefix . 'products_subtitle' => array(
                    'name' => EP4BOOKX_FIELD_SUBTITLE,
                    'value' => '',
                    'length' => zen_field_length(TABLE_PRODUCT_BOOKX_EXTRA_DESCRIPTION, 'products_subtitle')),
